...
def get_information(stock_symbol):
    ...
    if int(day)<11:
        month=int(month)-1
    start_price,start_date=get_start_price(int(year),int(month),11,stock_symbol+".tw")

    #再得到目前的價格跟時間
    year,month,day=take_time()
    current_price,final_update=get_final_price(int(year),int(month),int(day),stock_symbol+".tw")
